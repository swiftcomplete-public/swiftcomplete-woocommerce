  ...
  function initialiseSwiftcomplete() {
    swiftcomplete.runWhenReady(function () {
      ...
      if (autocompleteField) {
        ...
        var stateField = document.getElementById(type + '_state');
        var stateContainer = document.getElementById(type + '_state_field');
        if (stateField) {
          var stateFormat = returnStateCounty
            ? 'CeremonialCounty STATEABBREVIATION'
            : '';
          addressFields.push({
  ...
  function initialiseSwiftcomplete() {
    swiftcomplete.runWhenReady(function () {
      ...
      if (autocompleteField) {
        ...
        var adminStateField = document.getElementById('_' + type + '_state');
        if (adminStateField) {
          var adminStateFormat = returnStateCounty
            ? 'CeremonialCounty STATEABBREVIATION'
            : '';
          addressFields.push({
            field: adminStateField,
            format: adminStateFormat,
          });
        }
...
function initialiseSwiftcompleteBlocks(
  ...
  try {
    ...
    var blocksStateField = document.getElementById(type + '-state');
    if (blocksStateField) {
      var blocksStateFormat = returnStateCounty ? 'County' : '';
      addressFields.push({
        container: blocksStateField.parentNode,
        field: blocksStateField,
        format: blocksStateFormat,
      });
    }
    ...
    for (var i = 0; i < addressFields.length; i++)
      if (addressFields[i].container)
        addressFields[i].container.style.display =
          addressFields[i].field.id == 'swiftcomplete_what3words' ||
          fieldsVisible
            ? 'block'
            : 'none';
  }
